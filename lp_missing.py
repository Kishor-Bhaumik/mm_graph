...
class WandbLogger:
    def __init__(self, project_name="graph-link-prediction", run_name=None, config=None, use_logger=True):
...
def apply_missing_modalities(features, cfg, seed=None):
    """
    Apply missing modality simulation to node features.
    
    Args:
        features (torch.Tensor): Node features of shape (N, D)
        cfg (DictConfig): Configuration containing missing modality parameters
        seed (int): Random seed for reproducibility
        
    Returns:
        torch.Tensor: Modified features with missing modalities applied
        dict: Statistics about the missing modality application
    """
    if not cfg.use_missing_modalities or cfg.missing_percentage == 0.0:
        log.info("Missing modalities disabled or percentage is 0.0. Skipping modality masking.")
        return features, {}
    
    # Validate parameters
    assert 0.0 <= cfg.missing_percentage <= 1.0, f"missing_percentage must be between 0 and 1, got {cfg.missing_percentage}"
    assert cfg.missing_modality_type in ["image", "text", "both"], f"missing_modality_type must be 'image', 'text', or 'both', got {cfg.missing_modality_type}"
    assert cfg.missing_strategy in ["zero", "noise"], f"missing_strategy must be 'zero' or 'noise', got {cfg.missing_strategy}"
    ...
    N, D = features.shape
    assert D % 2 == 0, f"Feature dimension must be even to split into image/text, got {D}"
    
    # Calculate dimensions
    image_dim = D // 2
    text_dim = D // 2
    
    # Select nodes to apply missing modalities
    num_missing_nodes = int(N * cfg.missing_percentage)
    missing_node_indices = torch.randperm(N)[:num_missing_nodes]
    ...
    stats = {
        'num_nodes_affected': num_missing_nodes,
        'percentage_affected': cfg.missing_percentage,
        'modality_type': cfg.missing_modality_type,
        'strategy': cfg.missing_strategy,
        'total_nodes': N
    }
    
    if num_missing_nodes > 0:
        if cfg.missing_strategy == "zero":
            # Set features to zero
            if cfg.missing_modality_type == "image":
                modified_features[missing_node_indices, :image_dim] = 0.0
            elif cfg.missing_modality_type == "text":
                modified_features[missing_node_indices, image_dim:] = 0.0
            elif cfg.missing_modality_type == "both":
                modified_features[missing_node_indices, :] = 0.0
                
        elif cfg.missing_strategy == "noise":
            # Add Gaussian noise to features
            noise_mean = cfg.get('noise_mean', 0.0)
            noise_std = cfg.get('noise_std', 1.0)
            
            stats['noise_mean'] = noise_mean
            stats['noise_std'] = noise_std
            
            if cfg.missing_modality_type == "image":
                noise = torch.normal(noise_mean, noise_std, size=(num_missing_nodes, image_dim))
                modified_features[missing_node_indices, :image_dim] += noise
            elif cfg.missing_modality_type == "text":
                noise = torch.normal(noise_mean, noise_std, size=(num_missing_nodes, text_dim))
                modified_features[missing_node_indices, image_dim:] += noise
            elif cfg.missing_modality_type == "both":
                noise = torch.normal(noise_mean, noise_std, size=(num_missing_nodes, D))
                modified_features[missing_node_indices, :] += noise
    
    # Log statistics
    log.info(f"Applied missing modalities to {num_missing_nodes}/{N} nodes ({cfg.missing_percentage*100:.1f}%)")
    log.info(f"Modality type: {cfg.missing_modality_type}, Strategy: {cfg.missing_strategy}")
    if cfg.missing_strategy == "noise":
        log.info(f"Noise parameters: mean={stats.get('noise_mean', 0.0)}, std={stats.get('noise_std', 1.0)}")
    
    return modified_features, stats
            ...
            with torch.no_grad():
                ...
                if cfg.no_eval is False:
                    ...
                    if valid_result > best_valid_result:
                        ...
                        # if cfg.save_checkpoint:  # Add this condition
...
def main(cfg: DictConfig):
    ...
    verbose = True
    device = ('cuda:'+str(cfg.gpu_id) if cfg.mode == 'puregpu' else 'cpu') # use 'cuda' if GPU is available
    ...
    # Apply missing modalities simulation right after loading features
    original_features = g.ndata['feat']
    log.info(f"Original feature shape: {original_features.shape}")
    
    # Use the same seed as the general experiment for reproducibility
    experiment_seed = getattr(cfg, 'seed', 42)
    modified_features, missing_stats = apply_missing_modalities(original_features, cfg, seed=experiment_seed)
    
    # Update graph with modified features
    g.ndata['feat'] = modified_features
    log.info(f"Modified feature shape: {modified_features.shape}")
    ...
    # Add missing modality statistics to wandb config
    if missing_stats:
        wandb_config.update({
            'missing_modalities': missing_stats
        })
    
    # Use experiment name from config if available, otherwise default
    experiment_name = cfg.wandb_experiment_name +"_"+cfg.missing_modality_type+"_"+str(cfg.missing_percentage) 
    #getattr(cfg, 'wandb_experiment_name', f"{cfg.model_name}_{dataset_name}_{feat_name}")
    ...
    for run in range(cfg.runs):
        ...
        if cfg.use_logger:
            ...
            # Log missing modality statistics for this run
            if missing_stats:
                wandb_logger.log_metrics({
                    'missing_modalities/num_nodes_affected': missing_stats['num_nodes_affected'],
                    'missing_modalities/percentage_affected': missing_stats['percentage_affected'],
                    'missing_modalities/total_nodes': missing_stats['total_nodes'],
                    'run_id': run
                }, step=0)
    ...
    log.info(f"MRR: {mean_mrr:.4f} ± {std_mrr:.4f}")
    log.info(f"Hits@1: {mean_h1:.4f} ± {std_h1:.4f}")
    log.info(f"Hits@10: {mean_h10:.4f} ± {std_h10:.4f}")
    
    if missing_stats:
        log.info(f"Missing Modality Experiment Summary:")
        log.info(f"  - Nodes affected: {missing_stats['num_nodes_affected']}/{missing_stats['total_nodes']} ({missing_stats['percentage_affected']*100:.1f}%)")
        log.info(f"  - Modality type: {missing_stats['modality_type']}")
        log.info(f"  - Strategy: {missing_stats['strategy']}")
        if 'noise_mean' in missing_stats:
            log.info(f"  - Noise parameters: mean={missing_stats['noise_mean']}, std={missing_stats['noise_std']}")
